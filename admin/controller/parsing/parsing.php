...
class ControllerParsingParsing extends Controller {
	...
	public function index() {
		...
		<br>
		<a href="parsing.php?func=add_products&supplier=militarist&resetlinks">Обнулить сайтмап (заново)</a>&nbsp;&nbsp;|&nbsp;&nbsp;
  ...
  //import prices and products
	public function importCategory( $data) {
        ...
        if ( $data['name'] != '') {
                ...
                $r = $this->db->query( "SELECT c.category_id FROM `" . DB_PREFIX . "category` c
	...
	public function importProduct( $dataIn) {
		...
		if ( $dataIn['name'] != '') {
                ...
                $this->db->query("INSERT INTO " . DB_PREFIX . "product_to_layout SET product_id = '" . (int)$product_id . "', store_id = '0', layout_id = '1'");
